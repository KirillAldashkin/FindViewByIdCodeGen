...
static class Utils
{
    ...
    public static StringBuilder AppendComments(this StringBuilder sb, ClassToGen c) => sb
        .AppendLine("// Code below was generated by \"KiDev.AndroidAutoFinder\" code generator. This file contains:")
        .AppendLine($"//  Class: {c.FullName} [OnCreate = {c.OnCreateDefined}; Type = {c.ClassType}; AfterOnCreate = {c.AfterOnCreateDefined}] | SetContentView({c.ContentViewName})")
        .AppendLine(c.Fields.Select(f => $"//   Field: {f.Name} | Type: {f.FieldType} | FindViewById({f.FindName})"));
    ...
    public static StringBuilder AppendAutoFind(this StringBuilder sb, ClassToGen c) => sb
        ...
        .AppendLine(c.Fields.Select(f => $"\t\t\t{f.GetSourceString()}"))
        .AppendLine("\t\t}");
